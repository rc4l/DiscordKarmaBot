...
export const processInitialReactions = async (message: Message | PartialMessage, settings: ServerSettings) => {
	...
	const c = await Promise.all([determineEmoji(likeReaction, message?.guild?.emojis), determineEmoji(dislikeReaction, message?.guild?.emojis)]);
	const preferredLike = c[0] ?? LIKE;
	const preferredDislike = c[1] ?? DISLIKE;
	try {
...
const determineEmoji = async (preferredEmojiName: string, emojis: any) => {
	let e = null;
	...
	// Custom Emoji
		...
		if (numExists && emojis.cache.get(numExists[0])) {e = numExists[0];}
			...
			await emojis.fetch();
			if (numExists && emojis.cache.get(numExists[0])) {e = numExists[0];}
			// else {console.log('Cache could not find emoji with name: ' + preferredEmojiName + ' and number: ' + (numExists?.length ? numExists[0] : 'N/A'));}
		}
	}
	return e;
};